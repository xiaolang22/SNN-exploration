"""
Interactive combined analysis for in-vitro neural network output data.
...
3. Combine the three sheets into one 360-sample dataset.
4. Run 10-fold cross-validation for Random Forest, Ridge Classifier and
   Logistic Regression.
5. Create one result folder containing:
   - the combined-analysis accuracy figure
   - the accuracy table
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Select one Excel file and run combined three-sheet analysis."
    )
    parser.add_argument(
        "--file",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Parallel jobs. Default is 1 for Windows compatibility.",
    )
...
def evaluate_combined_data(
        ...
        rows.append(
            {
                ...
                "accuracy_std": float(np.std(scores, ddof=1)),
    ...
    return pd.DataFrame(rows).sort_values(
        by="accuracy_mean", ascending=False, ignore_index=True
    )


def unique_result_dir(output_root: Path, excel_stem: str) -> Path:
    output_root.mkdir(parents=True, exist_ok=True)
    base = output_root / f"{excel_stem}_combined_result"
    if not base.exists():
        base.mkdir()
        return base
    ...
    index = 2
    while True:
        candidate = output_root / f"{excel_stem}_combined_result_{index}"
...
def plot_combined_results(results: pd.DataFrame, output_path: Path) -> None:
    plt.rcParams.update(
        {
            "font.family": "Arial",
    ...
    for index, value in enumerate(results["accuracy_mean"]):
        ax.text(index, value + 0.025, f"{value:.2f}", ha="center", va="bottom", fontsize=9)
    ...
    ax.set_xlabel("Model")
    ax.set_title("Combined three-sheet 10-fold cross-validation", fontsize=10)
...
def main() -> None:
    ...
    data = [load_and_validate_sheet(excel_file, sheet_name) for sheet_name in selected_sheet_names]
    x_combined = np.vstack([item[0] for item in data])
    y_combined = np.concatenate([item[1] for item in data])

    models = build_models(args.random_state, args.n_jobs)
    result_df = evaluate_combined_data(
        models, x_combined, y_combined, args.random_state, args.n_jobs
    )

    result_dir = unique_result_dir(args.output_root, excel_file.stem)
    figure_path = result_dir / "combined_accuracy.png"
    accuracy_path = result_dir / "combined_accuracy_results.csv"
    excel_copy_path = result_dir / excel_file.name

    plot_combined_results(result_df, figure_path)
    ...
    print()
    print(f"Analyzed sheets: {', '.join(selected_sheet_names)}")
    print(f"Combined samples: {x_combined.shape[0]}")
