...
class StructureEvolver:
    ...
    def evaluate_topology(self, adjacency: np.ndarray, label: str, eval_index: int) -> CandidateResult:
        ...
        activity_score = self._activity_score(float(active_union.mean()))
    ...
    def _activity_score(self, active_fraction: float) -> float:
        target = self.structure_cfg.activity_target
        tol = max(1e-6, self.structure_cfg.activity_tolerance)
        score = 1.0 - abs(active_fraction - target) / tol
