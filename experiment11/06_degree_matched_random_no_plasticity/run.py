...
from structure import prepare_topology, summarize_topology


GROUP_CONFIGS = {
    "01_random_no_plasticity": {
        "label": "Random topology + no plasticity",
        "topology_mode": "random",
        "stdp_enabled": False,
    },
    "02_evolved_no_plasticity": {
        "label": "Evolved topology + no plasticity",
        "topology_mode": "evolved",
        "stdp_enabled": False,
    },
    "03_random_stdp": {
        "label": "Random topology + STDP",
        "topology_mode": "random",
        "stdp_enabled": True,
    },
    "04_evolved_stdp": {
        "label": "Evolved topology + STDP",
        "topology_mode": "evolved",
        "stdp_enabled": True,
    },
    "05_degree_matched_random_stdp": {
        "label": "Degree-matched random topology + STDP",
        "topology_mode": "degree_matched_random",
        "stdp_enabled": True,
    },
    "06_degree_matched_random_no_plasticity": {
        "label": "Degree-matched random topology + no plasticity",
        "topology_mode": "degree_matched_random",
        "stdp_enabled": False,
    },
}
...
def configure_group(cfg: Config, group_name: str) -> dict:
    if group_name not in GROUP_CONFIGS:
        raise ValueError(f"Unsupported experiment11 group: {group_name}")
    spec = GROUP_CONFIGS[group_name]
    cfg.plasticity.stdp.enabled = spec["stdp_enabled"]
    cfg.structure.topology_mode = spec["topology_mode"]
    return spec
...
def main():
    ...
    cfg = Config()
    group_name = experiment_dir.name
    group_spec = configure_group(cfg, group_name)
    topology_override, structure_summary = prepare_topology(cfg, output_dir)

    exp = Experiment(cfg, topology_override=topology_override)
    n_rounds = cfg.experiment.n_rounds
    reps = cfg.experiment.reps_per_mode

    print("=" * 50)
    print("  Experiment 11: Adaptive Structure Evolution")
    print("=" * 50)
    print(f"  Group: {group_spec['label']}")
    ...
    print(f"  Topology mode: {cfg.structure.topology_mode}")
    print(f"  STDP enabled: {cfg.plasticity.stdp.enabled}")
    ...
    if structure_summary is not None:
        print(f"  Evolved fitness: {structure_summary['selected_metrics']['fitness']:.4f}")
    print()
    ...
    topology_metrics = summarize_topology(exp.model.get_connectivity_mask(), exp.model.positions)

    results_data = {
        "group": {
            "name": group_name,
            "label": group_spec["label"],
            "topology_mode": cfg.structure.topology_mode,
        ...
        "structure_evolution": structure_summary,
        "classification": {
