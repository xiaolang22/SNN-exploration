...
class Experiment:
    def __init__(self, cfg: Config, topology_override: dict | None = None) -> None:
        self.cfg = cfg
        self.rng = np.random.default_rng(cfg.experiment.seed)
        self.model = ReservoirModel(cfg, self.rng, topology_override=topology_override)
    ...
    def warmup(self) -> tuple[np.ndarray, float]:
        ...
        use_plasticity = self.cfg.plasticity.stdp.enabled
        ...
        for t in range(total_steps):
            fired = self.model.step(plasticity=use_plasticity)
            if t >= start_record:
        ...
        for rnd in range(n_rounds):
            ...
            for mode_id in trial_plan:
                ...
                counts = self.model.run_segment(
                    exp.trial_duration_ms, pulse_currents, plasticity=use_plasticity,
