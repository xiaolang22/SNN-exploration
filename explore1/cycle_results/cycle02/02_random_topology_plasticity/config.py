...
class TopologyPlasticityParams:
    ...
    max_add_per_update: int = 32
    max_prune_per_update: int = 32
    max_edge_fraction_per_update: float = 0.0015
    min_pair_cooccurrence: int = 5
    delay_window_ms: tuple[float, float] = (2.0, 12.0)
    class_specificity_weight: float = 0.9
    pair_count_weight: float = 0.4
    consistency_weight: float = 1.2
    distance_penalty: float = 0.12
    inactive_quantile: float = 0.25
    protect_min_in_degree: int = 1
    protect_min_out_degree: int = 1
    max_candidate_distance_mm: float = 1.2
