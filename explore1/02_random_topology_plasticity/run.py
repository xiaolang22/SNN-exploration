...
def main():
    ...
    print("=" * 50)
    print("  Group: random topology + online topology plasticity")
    ...
    print("\nTopology updates:")
    print("+-------+--------+---------+-----------------+-------------+")
    print("| Round | Added  | Removed | Candidate pairs | Path length |")
    print("+-------+--------+---------+-----------------+-------------+")
    for i, upd in enumerate(result.topology_updates):
        tm = upd["topology_metrics"]
        print(f"|  {i+1:<4} | {upd['added']:6d} | {upd['removed']:7d} |"
              f" {upd['candidate_pairs']:15d} |    {tm['approx_path_length']:.4f}   |")
    print("+-------+--------+---------+-----------------+-------------+")

    results_data = {
        "group": "02_random_topology_plasticity",
        "config": {
            ...
            "topology_plasticity": {
                "enabled": cfg.topology_plasticity.enabled,
                "update_interval_rounds": cfg.topology_plasticity.update_interval_rounds,
                "max_add_per_update": cfg.topology_plasticity.max_add_per_update,
                "max_prune_per_update": cfg.topology_plasticity.max_prune_per_update,
                "max_edge_fraction_per_update": cfg.topology_plasticity.max_edge_fraction_per_update,
            },
        },
        "hotspots": {
        ...
        "topology_updates": result.topology_updates,
    }
