...
from .topology_plasticity import OnlineTopologyPlasticity
...
class ExperimentResult:
    ...
    topology_metrics: list[dict[str, float]] = field(default_factory=list)
    topology_updates: list[dict[str, object]] = field(default_factory=list)
...
class Experiment:
    def __init__(self, cfg: Config) -> None:
        ...
        self.topology_plasticity = OnlineTopologyPlasticity(cfg, self.model)
    ...
    def warmup(self) -> tuple[np.ndarray, float]:
        ...
        for t in range(total_steps):
            fired = self.model.step(plasticity=False)
            if t >= start_record:
    ...
    def run_rounds(self, callback=None) -> ExperimentResult:
        ...
        for rnd in range(n_rounds):
            ...
            for mode_id in trial_plan:
                ...
                counts, spike_steps = self.model.run_segment_with_spike_history(
                    exp.trial_duration_ms,
                    pulse_currents,
                    plasticity=False,
                ...
                round_spike_records.append(spike_steps)
                round_mode_labels.append(mode.label)
            ...
            self.result.topology_metrics.append(self.model.get_topology_metrics())
            if (rnd + 1) % self.cfg.topology_plasticity.update_interval_rounds == 0:
            ...
            self.result.topology_updates.append(update_summary)
            if callback:
