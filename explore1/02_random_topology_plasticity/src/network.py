...
class ReservoirModel:
    ...
    def _build_connectivity(self) -> None:
        ...
        self._refresh_connectivity_views()

    def _refresh_connectivity_views(self) -> None:
        n = self.n_total
        sources = self.edge_src
        targets = self.edge_tgt
        exc_edge_mask = self.edge_is_exc
        ...
        self.out_degree = np.bincount(sources, minlength=n).astype(np.int32)
        self.in_degree = np.bincount(targets, minlength=n).astype(np.int32)
    ...
    def run_segment(self, duration_ms: float, pulse_currents: dict[int, np.ndarray] | None = None,
        ...
        spike_counts, _ = self.run_segment_with_spike_history(
            duration_ms,
    ...
    def run_segment_with_spike_history(
        self,
        duration_ms: float,
        pulse_currents: dict[int, np.ndarray] | None = None,
        plasticity: bool = True,
        response_window_ms: tuple[float, float] | None = None,
        ...
        for t in range(n_steps):
            ...
            spike_history.append(fired.copy())
    ...
    def get_topology_metrics(self) -> dict[str, float]:
        dists = np.linalg.norm(
            self.positions[self.edge_src] - self.positions[self.edge_tgt], axis=1
        )
        graph = nx.from_edgelist(
            np.column_stack([self.edge_src, self.edge_tgt]).tolist()
        )
        if graph.number_of_nodes() == 0:
            clustering = 0.0
            largest_component_fraction = 0.0
            approx_path_length = 0.0
        else:
            clustering = nx.average_clustering(graph)
            largest_cc = max(nx.connected_components(graph), key=len)
            largest_component_fraction = len(largest_cc) / self.n_total
            subgraph = graph.subgraph(largest_cc).copy()
            if subgraph.number_of_nodes() > 1:
                approx_path_length = nx.average_shortest_path_length(subgraph)
            else:
                approx_path_length = 0.0

        return {
            "n_edges": int(self.n_edges),
            "avg_in_degree": float(self.in_degree.mean()),
            "avg_out_degree": float(self.out_degree.mean()),
            "std_in_degree": float(self.in_degree.std()),
            "std_out_degree": float(self.out_degree.std()),
            "mean_connection_distance_mm": float(dists.mean()) if len(dists) else 0.0,
            "clustering_coefficient": float(clustering),
            "largest_component_fraction": float(largest_component_fraction),
            "approx_path_length": float(approx_path_length),
        }
    ...
    def rewire_edges(
        ...
        remove_edge_ids: list[int],
    ) -> dict[str, int]:
        if not add_edges and not remove_edge_ids:
            return {"added": 0, "removed": 0}

        remove_mask = np.zeros(self.n_edges, dtype=bool)
        if remove_edge_ids:
            remove_mask[np.array(remove_edge_ids, dtype=np.int64)] = True

        keep_mask = ~remove_mask
        kept_src = self.edge_src[keep_mask]
        kept_tgt = self.edge_tgt[keep_mask]
        kept_w = self.edge_w[keep_mask]
        kept_is_exc = self.edge_is_exc[keep_mask]

        if add_edges:
            add_src = np.array([src for src, _ in add_edges], dtype=np.int32)
            add_tgt = np.array([tgt for _, tgt in add_edges], dtype=np.int32)
            wgt = self.cfg.weights
            add_w = np.exp(
                self.rng.normal(wgt.exc_mu, wgt.exc_sigma, size=len(add_edges))
            )
            add_is_exc = np.ones(len(add_edges), dtype=bool)
            self.edge_src = np.concatenate([kept_src, add_src])
            self.edge_tgt = np.concatenate([kept_tgt, add_tgt])
            self.edge_w = np.concatenate([kept_w, add_w])
            self.edge_is_exc = np.concatenate([kept_is_exc, add_is_exc])
        else:
            self.edge_src = kept_src
            self.edge_tgt = kept_tgt
            self.edge_w = kept_w
            self.edge_is_exc = kept_is_exc

        self.n_edges = len(self.edge_src)
        self._refresh_connectivity_views()
        return {"added": len(add_edges), "removed": int(remove_mask.sum())}
