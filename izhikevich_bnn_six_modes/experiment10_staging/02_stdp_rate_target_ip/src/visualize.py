...
def plot_ip_metric_evolution(result: ExperimentResult, output_dir: Path) -> None:
    if not result.ip_metrics:
        return

    _setup_style()
    rounds = np.arange(1, len(result.ip_metrics) + 1)
    fig, axes = plt.subplots(3, 1, figsize=(6, 7), sharex=True)

    axes[0].plot(rounds, [m["kl_divergence"] for m in result.ip_metrics], "o-", color=COLORS[0])
    axes[0].set_ylabel("KL divergence")

    axes[1].plot(rounds, [m["silent_neuron_ratio"] for m in result.ip_metrics], "o-", color=COLORS[1])
    axes[1].plot(rounds, [m["burst_like_trial_ratio"] for m in result.ip_metrics], "s--", color=COLORS[3])
    axes[1].legend(["Silent neuron ratio", "Burst-like trial ratio"], loc="upper right")
    axes[1].set_ylabel("Ratio")

    axes[2].plot(rounds, [m["mean_rate_hz"] for m in result.ip_metrics], "o-", color=COLORS[2])
    axes[2].set_xlabel("Training round")
    axes[2].set_ylabel("Mean rate (Hz)")
    axes[2].set_xticks(rounds)

    fig.tight_layout()
    _save(fig, output_dir, "ip_metric_evolution")


def plot_rate_distribution(result: ExperimentResult, output_dir: Path, target_mu_hz: float) -> None:
    if not result.intrinsic_snapshots:
        return

    final_key = sorted(result.intrinsic_snapshots.keys())[-1]
    final_rates = result.intrinsic_snapshots[final_key]["rates_hz"]
    upper = max(float(final_rates.max()), 5.0 * target_mu_hz, 1.0)
    x = np.linspace(0.0, upper, 200)
    pdf = (1.0 / max(target_mu_hz, 1e-9)) * np.exp(-x / max(target_mu_hz, 1e-9))

    _setup_style()
    fig, ax = plt.subplots(figsize=(5.5, 3.5))
    ax.hist(final_rates, bins=40, density=True, alpha=0.65, color=COLORS[0], label="Observed rates")
    ax.plot(x, pdf, color=COLORS[3], linewidth=1.6, label=f"Target Exp(mu={target_mu_hz:.1f} Hz)")
    ax.set_xlabel("Estimated firing rate (Hz)")
    ax.set_ylabel("Density")
    ax.legend()
    fig.tight_layout()
    _save(fig, output_dir, "rate_distribution")


def plot_intrinsic_parameter_distribution(result: ExperimentResult, output_dir: Path) -> None:
    if not result.intrinsic_snapshots:
        return

    final_key = sorted(result.intrinsic_snapshots.keys())[-1]
    bias_current = result.intrinsic_snapshots[final_key]["bias_current"]
    input_gain = result.intrinsic_snapshots[final_key]["input_gain"]

    _setup_style()
    fig, axes = plt.subplots(1, 2, figsize=(8, 3.2))
    axes[0].hist(bias_current, bins=40, color=COLORS[0], alpha=0.75, density=True)
    axes[0].set_xlabel("Bias current")
    axes[0].set_ylabel("Density")
    axes[0].set_title("Final bias distribution")

    axes[1].hist(input_gain, bins=40, color=COLORS[1], alpha=0.75, density=True)
    axes[1].set_xlabel("Input gain")
    axes[1].set_ylabel("Density")
    axes[1].set_title("Final input gain distribution")

    fig.tight_layout()
    _save(fig, output_dir, "intrinsic_parameter_distribution")
